...
class mod_hacks extends module {
	
	function init () {
		global $CFG;
		
		$this->extable = $CFG['db_pfx'].'_res_hacks';
		...
		$this->file_restrictions = array(
			'file'	=> array(
				...
				'ext'	=> array('ZIP','RAR'),
				'size'	=> array(0, 1024*1024*100, '0B', '100MB'),
			),
			'thumbnail'	=> array(
	...
	function update_block ($module, $time, $limit) {
		...
		// Sub Template
		$STPL = new template;
		$shtml = $STPL->useTemplate('mod_hacks');
		...
		$RES->query_condition("fg.keyword = 'BASE_ROM'");
		$RES->query_condition("r.queue_code = '0'");
		$RES->getByType($module['mid']);
			
		$num_items = 0;
		//$output = $shtml->news_update_block_header( $module['full_name'] ); 
		$output = "<div class='newsstrip'>Hacks, Mods</div><table class='sformtable' cellspacing='1'>";
		...
		while ($RES->nextItem())
		{
			...
			//$RES->data['thumbnail'] = "<img src='thumbnail/7/{$RES->data['thumbnail']}' />";
	...
	function common_data_check () {
		...
		if (empty($IN['cat1']) || empty($IN['cat2']) || empty($IN['cat3']) || empty($IN['cat4']))
			$this->error_save("You must chose a value for the completion, base rom, rom region, and hack type categories.", 'submit');
	...
	function common_prep_data (&$row) {
		...
		// This is where Hypernova's scoring system would go if it didn't have major flaws. Check games.php for more info.
	...
	function common_edit_prep_data (&$row) {
		...
		$data['cat2'] = $this->make_catset('BASE_ROM', $access, $selected);
		$data['cat3'] = $this->make_catset('ROM_REGION', $access, $selected);
		$data['cat4'] = $this->make_catset('HACK_TYPE', $access, $selected);
	...
	function common_view_prep_data (&$row) {
		...
		if (!empty($IN['id'])) {
			...
			while ($arow = $DB->fetch_row()) {
				...
				if ($arow['fhit'] > 0 && $arow['keyword'] == 'BASE_ROM')
					$data['base_rom'] = $arow['name'];
				if ($arow['fhit'] > 0 && $arow['keyword'] == 'ROM_REGION')
					$data['region'] = $arow['name'];
				if ($arow['fhit'] > 0 && $arow['keyword'] == 'HACK_TYPE')
					$data['hack_type'] = $arow['name'];
			}
		}
	...
	function resdb_prep_page_data (&$res) {
		...
		if ($STD->modules->bound_child($res['type'], 'res_reviews')) {
			...
			$mod_html = $STD->template->useTemplate('mod_hacks');
	...
	function common_update_data () {
		...
		$auxdata['cat_base_rom'] = $IN['cat2'];	
		$auxdata['cat_hack_region'] = $IN['cat3'];
		$auxdata['cat_hack_type'] = $IN['cat4'];
	...
	function user_update_submit_data () {
		...
		$values = array($auxdata['cat_completion'], $auxdata['cat_base_rom'], $auxdata['cat_hack_region'], $auxdata['cat_hack_type']);
