...
foreach ($data as $dat) {
		// rowcell2
		$ret .= "<tr>
		<td width=\"12%\" class=\"rowtitle\" align=\"right\"><a href=\"{$dat['uidurl']}\">{$dat['name']}</a></td>
		<td class=\"rowcell2 canquote\" onClick=\"quote('{$dat['name']}', {$dat['id']});\">{$dat['message']}</td>
		<td width=\"23%\" class=\"rowtitle\">{$dat['date']}<div style=\"display:none\" id=\"msg{$dat['id']}\">{$dat['raw']}</div>";
		if ($dat['uid'] == $uid) 
